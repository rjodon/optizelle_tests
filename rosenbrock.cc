...
//---Objective0---
// Squares its input
template <typename Real> Real sq(Real x) { return x * x; }

// Define the Rosenbrock function where
//
// f(x,y)=(1-x)^2+100(y-x^2)^2
//
struct Rosenbrock : public Optizelle::ScalarValuedFunction<double, Optizelle::Rm> {
...
public:
    ...
    void eval(X_Vector const & dx,X_Vector & result) const { 
        ...
        result[1]=one_over_det*(400.*x[0]*dx[0]+(1200.*x[0]*x[0]-400.*x[1]+2.)*dx[1]);
    }
};
...
int main(int argc, char *argv[]) {
    ...
    auto fname = argv[1]; //---State0---
    // Generate an initial guess for Rosenbrock
    auto x = std::vector<double>{-1.2, 1.};
    // Create an unconstrained state based on this vector
    Optizelle::Unconstrained<double, Optizelle::Rm>::State::t state(x); //---State1---
    //---Parameters0---
    // Read the parameters from file
    Optizelle::json::Unconstrained<double, Optizelle::Rm>::read(fname, state); //---Parameters1---
    //---Functions0---
    ...
    std::cout << "The optimal point is: (" << state.x[0] << ',' << state.x[1] << ')'
              << std::endl;
    //---Extract1---
    // Write out the final answer to file
    Optizelle::json::Unconstrained<double, Optizelle::Rm>::write_restart("solution.json", state);
